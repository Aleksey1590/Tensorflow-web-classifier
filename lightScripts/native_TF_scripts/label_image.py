...
def run_graph(image_data, labels, input_layer_name, output_layer_name,
  ...
  with tf.Session() as sess:
    ...
    predictionArray = (-np.sort(-predictions, axis=None))
    # labelsSorted = 
    if (predictionArray[0]>=0.6):
      print ("Im confident with this one")
      # print (imageName, predictionArray[0])
      # print (imageName, labels[0])
      ...
      # print("Score: ", score)
      # print('%s (score = %.5f)' % (human_string, score))
      # copyFileTo(imageName, labels[top_k[0]])
    elif(predictionArray[0]<0.6 and predictionArray[0]>=0.2):
      print ("Not confident")
      # print (imageName, predictionArray[0])
      # print (imageName, labels[0])
      # print (imageName, labels[1])
      # for node_id in top_k:
      #   human_string = labels[node_id]
      #   score = predictions[node_id]
        # print("Score: ", score)
        # print('%s (score = %.5f)' % (human_string, score))
      # copyFileTo(imageName, labels[top_k[0]])
      # copyFileTo(imageName, labels[top_k[1]])
    else:
      print ("Misclassified file: ", labels)
      # copyFileTo(imageName, "other")
      # for node_id in top_k:
      #   human_string = labels[node_id]
      #   score = predictions[node_id]
        # print("Score: ", score)
...
def copyFileTo(fileSource, imageGenre):
  pass
  # See local file. 
